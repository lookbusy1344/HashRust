...
fn test_parse_hash_algorithm_valid() {
    assert_eq!(parse_hash_algorithm(Some("SHA3-256")).unwrap(), HashAlgorithm::SHA3_256);
    assert_eq!(parse_hash_algorithm(Some("MD5")).unwrap(), HashAlgorithm::MD5);
    assert_eq!(parse_hash_algorithm(Some("CRC32")).unwrap(), HashAlgorithm::CRC32);
    assert_eq!(parse_hash_algorithm(Some("WHIRLPOOL")).unwrap(), HashAlgorithm::Whirlpool);
}
...
fn test_parse_hash_encoding_valid() {
    assert_eq!(parse_hash_encoding(Some("Hex")).unwrap(), OutputEncoding::Hex);
    assert_eq!(parse_hash_encoding(Some("Base64")).unwrap(), OutputEncoding::Base64);
    assert_eq!(parse_hash_encoding(Some("Base32")).unwrap(), OutputEncoding::Base32);
}

#[test]
fn test_parse_hash_encoding_default() {
    assert_eq!(parse_hash_encoding(None).unwrap(), OutputEncoding::Unspecified);
    assert_eq!(parse_hash_encoding(Some("")).unwrap(), OutputEncoding::Unspecified);
}

#[test]
fn test_hash_algorithm_from_str() {
    assert_eq!(HashAlgorithm::from_str("sha3-256").unwrap(), HashAlgorithm::SHA3_256);
    assert_eq!(HashAlgorithm::from_str("SHA2").unwrap(), HashAlgorithm::SHA2_256);
